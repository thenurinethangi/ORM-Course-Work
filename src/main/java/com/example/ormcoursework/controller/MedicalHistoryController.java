...
public class MedicalHistoryController implements Initializable {
    ...
    void addBtnOnAction(ActionEvent event) {
        ...
        String id = medicalHistoryIdLabel.getText();
        String patientId = patientIdCmb.getSelectionModel().getSelectedItem();
        String diagnosis = diagnosisField.getText();
        String bloodPressure = bloodPressureField.getText();
        String sleepingPattern = sleepingPatternCmb.getSelectionModel().getSelectedItem();
        String therapyResponse = responseCmb.getSelectionModel().getSelectedItem();
        String condition = conditionCmb.getSelectionModel().getSelectedItem();

        if(id.isEmpty() || patientId==null || diagnosis.isEmpty() || bloodPressure.isEmpty() || sleepingPattern==null || therapyResponse==null || condition==null ){

            notification("Please Fill In All Field To Add A New Medical History");
            return;
        }

        boolean diagnosisValidation = UserInputValidation.checkTextValidation(diagnosis);
        boolean bloodPressureValidation = UserInputValidation.checkBloodPressureValidation(bloodPressure);

        userInputValidate(diagnosisValidation,bloodPressureValidation);

        if(diagnosisValidation && bloodPressureValidation){

            Patient patient = DTOToEntity.ToPatientEntity(medicalHistoryBO.getPatientDetailsById(patientId));

            MedicalHistoryDTO medicalHistoryDTO = new MedicalHistoryDTO(id,patient,diagnosis,bloodPressure,sleepingPattern,therapyResponse,condition,new Date());

            boolean isSave = medicalHistoryBO.add(medicalHistoryDTO);

            if(!isSave){
                notification("Something Went Wrong With Adding A New Medical History For Patient ID: "+patientId+" , Please Try Again Later");
                clean();
                return;
            }

            notification("Successfully Added The New Medical History For Patient ID: "+patientId);
            clean();

        }
        else{
            notification("Please Enter Valid Data Into The Fields To Add New Medical History");
        }
    }
    ...
    void patientIdCmbOnAction(ActionEvent event) {
        ...
        String id = patientIdCmb.getSelectionModel().getSelectedItem();

        if(id==null){
            return;
        }

        PatientDTO patientDTO = medicalHistoryBO.getPatientDetailsById(id);
        if(patientDTO==null){
            return;
        }
    ...
    void updateBtnOnAction(ActionEvent event) {
        ...
        if(diagnosisValidation && bloodPressureValidation){
            ...
            boolean isUpdate = medicalHistoryBO.update(medicalHistoryDTO);

            if(!isUpdate){
                notification("Something Went Wrong With Updating A Selected Medical History For Patient ID: "+patientId+" , Please Try Again Later");
                clean();
                return;
            }

            notification("Successfully Updated The Medical History ID: "+id);
            clean();

        }
        else{
            notification("Please Enter Valid Data Into The Fields To Update A Medical History");
        }
    }
                ...
                public void updateItem(String item, boolean empty) {
                    ...
                    if (empty) {
                        ...
                        delete.setOnMouseClicked((MouseEvent event) -> {
                            ...
                            MedicalHistoryTM selectedItem = table.getSelectionModel().getSelectedItem();

                            Alert alert = new Alert(Alert.AlertType.CONFIRMATION,"Are You Sure You Want To Delete The Medical History ID: "+selectedItem.getMedicalHistoryId(),ButtonType.YES,ButtonType.CANCEL);
                            Optional<ButtonType> result = alert.showAndWait();

                            if(result.isPresent() && (result.get() == ButtonType.YES)){

                                MedicalHistoryDTO medicalHistoryDTO = TMToDTO.ToMedicalHistoryDTO(selectedItem);
                                boolean isDelete = medicalHistoryBO.delete(medicalHistoryDTO);

                                if(isDelete){
                                    notification("Successfully Deleted The Medical History ID: "+selectedItem.getMedicalHistoryId());
                                }
                                else{
                                    notification("Failed Delete The Medical History ID: "+selectedItem.getMedicalHistoryId());
                                }
                            }
                        ...
                        update.setOnMouseClicked((MouseEvent event) -> {
                            ...
                            medicalHistoryIdLabel.setText(selectedItem.getMedicalHistoryId());
                            patientIdCmb.getSelectionModel().select(selectedItem.getPatientId());
                            diagnosisField.setText(selectedItem.getPrimaryDiagnosis());
                            bloodPressureField.setText(selectedItem.getBloodPressure());
                            sleepingPatternCmb.getSelectionModel().select(selectedItem.getSleepingHabit());
                            responseCmb.getSelectionModel().select(selectedItem.getTherapyResponse());
                            conditionCmb.getSelectionModel().select(selectedItem.getCondition());
