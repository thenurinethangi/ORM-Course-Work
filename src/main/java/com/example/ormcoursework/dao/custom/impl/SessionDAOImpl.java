...
public class SessionDAOImpl implements SessionDAO {
    ...
    @Override
    public boolean add(TherapySession therapySession) {
        return false;
    }
    ...
    public List<TherapySession> getAllTodaySessions() {
        ...
        try{
            ...
            DateTimeFormatter formatter = DateTimeFormatter.ofPattern("MMMM yyyy EEEE d", Locale.ENGLISH);
    ...
    @Override
    public List<TherapySession> getThisMonthSessionsList() {

        Session session = FactoryConfiguration.getInstance().getSession();

        LocalDate today = LocalDate.now();
        DateTimeFormatter dateTimeFormatter = DateTimeFormatter.ofPattern("MMMM yyyy",Locale.ENGLISH);
        String yearAndMonth = today.format(dateTimeFormatter);

        try{
            List<TherapySession> therapySessions = session.createQuery("from TherapySession",TherapySession.class).list();
            List<TherapySession> thisMonthSessionsList = new ArrayList<>();

            for (TherapySession x : therapySessions){
                String y = x.getDate();
                String[] arr = y.split(" ");
                String z = arr[0]+" "+arr[1];

                if(z.equals(yearAndMonth)){
                    thisMonthSessionsList.add(x);
                }
            }

            return thisMonthSessionsList;
